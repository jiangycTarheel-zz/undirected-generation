...
from .decoding_utils import _evaluate_batch_by_order, calculate_bleu_scores, _evaluate_batch_while_generate_order, \
    _evaluate_batch_while_generate_order_beam, _evaluate_order_model_on_batches
from .pybleu import PyBleuScorer
from src.data.dictionary import Dictionary
from src.rl.rl_utils import *
...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        if self.epoch_size == -1:
            assert False  # Just avoid this temporarily
            self.epoch_size = len(self.data['para'][('de', 'en')]['train'])
    ...
    def iter(self):
        ...
        self.print_stats()
        if self.params.save_ckpt_period is not None and self.n_iter % self.params.save_ckpt_period == 0:
    ...
    def mask_out(self, x, lengths):
        ...
        if params.sample_alpha == 0:
            if params.word_pred_uniform:
                ...
                one_pos = one_pos.reshape(x.shape[1], 4, 2)
    ...
    def generate_batch(self, lang1, lang2, name):
        ...
        return x, x1, x2, lengths, positions, langs, (None, None) if lang2 is None else (len1, len2)
    ...
    def save_checkpoint(self):
        ...
        checkpoint_path = os.path.join(self.params.dump_path, 'checkpoint_epoch'+str(self.epoch)+'+iter'+str(self.n_total_iter)+'.pth')
    ...
    def end_epoch(self, scores):
        ...
        if self.stopping_criterion is not None and (self.params.is_master or not self.stopping_criterion[0].endswith('_mt_bleu')):
            ...
            assert metric in scores, (metric, scores)
            factor = 1 if biggest else -1
    ...
    def clm_step(self, lang1, lang2, lambda_coeff):
        ...
        x, _, _, lengths, positions, langs, _ = self.generate_batch(lang1, lang2, 'causal')
    ...
    def mlm_step(self, lang1, lang2, lambda_coeff):
        ...
        # generate batch / select words to predict
        x, _, _, lengths, positions, langs, _ = self.generate_batch(lang1, lang2, 'pred')
        x, lengths, positions, langs, _ = self.round_batch(x, lengths, positions, langs)
...
class SingleTrainer(Trainer):
    ...
    def __init__(self, model, data, params, module='model'):
...
class SingleOrderTrainer(SingleTrainer):
    def __init__(self, model, order_model, data, params):
        super().__init__(model, data, params, module='order_model')
        self.MODEL_NAMES = ['order_model']
        self.order_model = order_model
        self.bleu_scorer = PyBleuScorer(case_insensitive=True, smooth=True)
        dico_id2word = self.data['dico'].id2word
        dico_word2id = self.data['dico'].word2id
        dico_counts = self.data['dico'].counts
        self.dico = Dictionary(dico_id2word, dico_word2id, dico_counts)
        self.optimizers = {'order_model': self.get_optimizer_fp('order_model')}
        self.reload_order_checkpoint()
        for param_group in self.optimizers['order_model'].param_groups:
            param_group['num_updates'] = 0

    def reload_order_checkpoint(self):
        """
        Reload a checkpoint if we find one.
        """
        #checkpoint_path = os.path.join(self.params.dump_path[:-10], 'checkpoint.pth')
        checkpoint_path = os.path.join(self.params.dump_path, 'checkpoint.pth')
        if not os.path.isfile(checkpoint_path):
            if self.params.reload_order_checkpoint == '':
                return
            else:
                checkpoint_path = self.params.reload_order_checkpoint
                assert os.path.isfile(checkpoint_path)
        logger.warning('Reloading checkpoint from %s ...' % checkpoint_path)
        data = torch.load(checkpoint_path, map_location=lambda storage, loc: storage.cuda(self.params.local_rank))
    ...
    def freeze_transformer(self, module):
        assert module in ['model', 'encoder', 'decoder', 'order_model']
        for _pname, _p in getattr(self, module).named_parameters():
            if 'order_pred' not in _pname and 'rl_baseline_fn' not in _pname:
                _p.requires_grad = False
            # print(_pname, _p.requires_grad)

    def unfreeze_transformer(self, module):
        assert module in ['model', 'encoder', 'decoder', 'order_model']
        for _pname, _p in getattr(self, module).named_parameters():
            if 'embeddings' in _pname or 'layer_norm_emb' in _pname \
            or 'attentions.0' in _pname or 'attentions.1' in _pname or 'attentions.2' in _pname or 'attentions.3' in _pname \
            or 'layer_norm1.0' in _pname or 'layer_norm1.1' in _pname or 'layer_norm1.2' in _pname or 'layer_norm1.3' in _pname \
            or 'layer_norm2.0' in _pname or 'layer_norm2.1' in _pname or 'layer_norm2.2' in _pname or 'layer_norm2.3' in _pname \
            or 'ffns.0' in _pname or 'ffns.1' in _pname or 'ffns.2' in _pname or 'ffns.3' in _pname:
                _p.requires_grad = False
            else:
    ...
    def get_optimizer_fp(self, module):
        ...
        if not self.params.separate_order_model:
            for _pname, _p in getattr(self, module).named_parameters():
                if 'order_pred' not in _pname and 'rl_baseline_fn' not in _pname:
                    _p.requires_grad = False
        optimizer = get_optimizer(getattr(self, module).parameters(), self.params.optimizer)
    ...
    def mask_out_all_trg(self, x, lengths):
        """
        Decide of random words to mask out, and what target they get assigned.
        """
        params = self.params
        ...
        if params.sample_alpha == 0:
            if params.word_pred_uniform:
                ...
                while True not in pred_mask:
                    # reset masks for the other side
                    for batch_idx in range(bs):
                        if params.rl_direction == 'src2trg':
                            pred_mask[src_len[batch_idx]+1:lengths[batch_idx]-1,batch_idx] = 1
                        else:
                            pred_mask[1:src_len[batch_idx]-1, batch_idx] = 1
        ...
        return x, _x_real, pred_mask, src_len, tgt_len

    def mask_out_with_order_model(self, x, lengths, sampled_order):
        """
        Decide of random words to mask out, and what target they get assigned.
        """
        params = self.params
        ...
        if params.sample_alpha == 0:
            if params.word_pred_uniform:
                ...
                # generate random mask
                pred_mask = np.ones((slen, bs), dtype=bool)
                for bs_i in range(bs):
                    sampled_order_i = sampled_order[bs_i][sampled_order[bs_i] != -100]
                    rand_cut = np.random.randint(0, sampled_order_i.size(0) + 1)

                    for tok_i in range(rand_cut):
                        pred_mask[sampled_order_i[tok_i], bs_i] = False
                ...
                for batch_idx in range(bs):
                    if side[batch_idx] == 0:
                        ...
                        pred_mask[:src_len[batch_idx]+1, batch_idx] = 0
                    else:
                        sys.exit("something is wrong")
            else:
    ...
    def order_pred_rl_step(self, lang1, lang2, lambda_coeff):
        """
        Masked word prediction step.
        MLM objective is lang2 is None, TLM objective otherwise.
        """
        ...
        # generate batch / select words to predict
        x, src_x, tgt_x, lengths, positions, langs, _ = self.generate_batch(lang1, lang2, 'pred')
        #x, lengths, positions, langs, _ = self.round_batch(x, lengths, positions, langs)
        x_masked, y, pred_mask, src_len, tgt_len = self.mask_out_all_trg(x.clone(), lengths)

        # cuda
        x_masked_cuda, y_cuda, pred_mask_cuda, lengths_cuda, positions_cuda, langs_cuda, src_len_cuda, tgt_len_cuda = to_cuda(
            x_masked, y, pred_mask, lengths, positions, langs, src_len, tgt_len)

        ### Decode the model to get the reward
        model.eval()
        order_model.train()

        if params.mlm_with_rl:
            self.freeze_transformer('order_model')

        # Decode the model by this order, and get the BLEU scores
        with torch.autograd.set_detect_anomaly(True):

            sampled_ids, sampled_order, order_logits, order_mask, state_values, scores_argmax_tok, _, _ = _evaluate_batch_while_generate_order(
                model, order_model, params, x_masked_cuda.clone(), lengths_cuda, positions_cuda, langs_cuda, src_len, tgt_len,
                params.rl_direction,
                separate_order_model=params.separate_order_model,
                rl_baseline=params.rl_baseline,
                restrain_action_space=params.restrain_action_space,
                top_k=params.top_k_for_sampling_token,
            )

            scores_argmax_tok = torch.exp(scores_argmax_tok)
            sampled_bleu = calculate_bleu_scores(sampled_ids, src_x, tgt_x, src_len, tgt_len, params,
                                                 params.rl_direction, self.bleu_scorer, self.dico)
            sampled_bleu = [_bleu / 100.0 for _bleu in sampled_bleu]

            if params.reward_fn == 'bleu':
                reward = torch.tensor(sampled_bleu).cuda()
            elif params.reward_fn == 'model_scores':
                reward = scores_argmax_tok
            else:
                raise NotImplementedError

            order_mask_cuda = order_mask.cuda()

            if params.rl_baseline == 'l2r_critic':
                # Decode the model left2right, and get the BLEU scores
                l2r_order = torch.arange(tgt_len.max() - 2).unsqueeze(0)
                l2r_order = l2r_order.expand(x.size(1), l2r_order.size(1))

                # Decode the model by this order, and get the BLEU scores
                l2r_decoded = _evaluate_batch_by_order(
                    model, params, x_masked.clone(), lengths, positions, langs, src_len, tgt_len,
                    params.rl_direction, l2r_order, cuda=(not params.generation_model_cpu),
                    separate_order_model=params.separate_order_model
                )
                pred_critic = calculate_bleu_scores(l2r_decoded, src_x, tgt_x, src_len, tgt_len, params, params.rl_direction,
                                                 self.bleu_scorer, self.dico)
                pred_critic = [_bleu / 100.0 for _bleu in pred_critic]
                sampled_bleu, pred_critic = torch.tensor(sampled_bleu).cuda(), torch.tensor(pred_critic).unsqueeze(1).cuda()
                adv, discounted_reward = compute_advantage(sampled_bleu, pred_critic, order_mask.cuda(),
                                                           params.discount_factor)
            elif params.rl_baseline == 'self_critic':
                greedy_ids, greedy_order, _, _, _, _, _, _ = _evaluate_batch_while_generate_order(
                    model, order_model, params, x_masked_cuda.clone(), lengths_cuda, positions_cuda, langs_cuda,
                    src_len, tgt_len, params.rl_direction,
                    separate_order_model=params.separate_order_model, rl_baseline=params.rl_baseline,
                    decode_order_strategy='top1'
                )
                pred_critic = calculate_bleu_scores(greedy_ids, src_x, tgt_x, src_len, tgt_len, params,
                                                     params.rl_direction, self.bleu_scorer, self.dico)
                pred_critic = [_bleu / 100.0 for _bleu in pred_critic]
                sampled_bleu, pred_critic = torch.tensor(sampled_bleu).cuda(), torch.tensor(pred_critic).unsqueeze(1).cuda()
                adv, discounted_reward = compute_advantage(sampled_bleu, pred_critic, order_mask.cuda(),
                                                           params.discount_factor)
            elif params.rl_baseline == 'state_value':
                pred_critic = torch.cat(state_values, dim=1).cuda()
                #sampled_bleu = torch.tensor(sampled_bleu).cuda()
                adv, discounted_reward = compute_advantage(reward, pred_critic, order_mask.cuda(), params.discount_factor)
            elif params.rl_baseline == 'TD_adv':
                pred_critic = torch.cat(state_values, dim=1).cuda()
                sampled_bleu = torch.tensor(sampled_bleu).cuda()
                raise NotImplementedError
            else:
            ...
            loss = order_model('predict_get_loss', logits=order_logits, labels=sampled_order.cuda(),
                               advantage=adv.cuda(), mask=order_mask.cuda())
            print(sampled_order[0])
            print(sampled_bleu[0])
            print("advantage")
            print(adv[0])
            print("critic:")
            print(pred_critic[0])

            if params.rl_baseline in ['state_value', 'TD_adv']:
                mse_loss = F.mse_loss(pred_critic, discounted_reward.half(), reduction='none')
                mse_loss = (mse_loss * order_mask_cuda).sum() / order_mask_cuda.sum()
                loss = loss + mse_loss

            if params.entropy_penalty:
                dec_len = order_logits.size(2)
                new_p = F.softmax(order_logits.reshape(-1, dec_len), dim=-1)
                new_p_wo_zero = new_p.clone()
                new_p_wo_zero[torch.abs(new_p_wo_zero) == 0.] = 0.001
                new_logp = torch.log(new_p_wo_zero)
                total_entropy = - new_p * new_logp * order_mask_cuda.reshape(-1).unsqueeze(1)
                entropy = torch.sum(total_entropy) / torch.sum(order_mask_cuda)
                loss = loss - params.entropy_coeff * entropy
                print("entropy:", entropy)

        # optimize
        self.optimize(loss, 'order_model')

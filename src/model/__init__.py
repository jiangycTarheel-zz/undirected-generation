...
from .transformer import DECODER_ONLY_PARAMS, TransformerModel, TransformerOrderPredModel  # , TRANSFORMER_LAYER_PARAMS
...
def build_model(params, dico, pred_order=False, to_gpu=True):
    """
    Build model.
    """
    if params.encoder_only:
        # build
        if pred_order:
            model = TransformerOrderPredModel(params, dico, is_encoder=True, with_output=True)
        else:

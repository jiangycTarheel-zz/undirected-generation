...
def set_dico_parameters(params, data, dico):
    ...
    if hasattr(params, 'bos_index'):
        assert params.n_words == n_words, (params.n_words, n_words)
...
def load_para_data(params, data, eval_max_length=None):
    """
    Load parallel data.
    """
    data['para'] = {}
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if splt == 'train':
                ...
                if eval_max_length is not None:
                    dataset.remove_long_sentences(eval_max_length)
...
def check_data_params(params):
    ...
    # order prediction steps
    order_pred_rl_steps = [s.split('-') for s in params.order_pred_rl_steps.split(',') if len(s) > 0]
    params.order_pred_rl_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in order_pred_rl_steps]
    assert all([(l1 in params.langs) and (l2 in params.langs or l2 is None) for l1, l2 in params.order_pred_rl_steps])
    assert len(params.order_pred_rl_steps) == len(set(params.order_pred_rl_steps))

    order_pred_sl_steps = [s.split('-') for s in params.order_pred_sl_steps.split(',') if len(s) > 0]
    params.order_pred_sl_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in order_pred_sl_steps]
    assert all([(l1 in params.langs) and (l2 in params.langs or l2 is None) for l1, l2 in params.order_pred_sl_steps])
    assert len(params.order_pred_sl_steps) == len(set(params.order_pred_sl_steps))
    ...
    params.para_dataset = {
        ...
        if ((src < tgt and tgt != 'ro') or (src > tgt and src == 'ro'))
...
def load_data(params, eval_max_length=None):
    """
    Load monolingual data.
    ...
    # parallel datasets
    load_para_data(params, data, eval_max_length)

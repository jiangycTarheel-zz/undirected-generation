...
best_deen_order_model="models/learned_order_deen_uniform_4gpu/00_maxlen30_minlen5_bsz32"

best_deen_order_model=${best_deen_order_model}/${2}
gen_type=${3:-src2trg}
gpuid=${4:-0}
split=${5:-valid}
n_iter=${6:-1}
...
function greedy_1iter() {
  python3 -W ignore adaptive_gibbs_sampler_simple_with_predicted_order.py --use_data_length --num_topk_lengths 4 --split $split --model_path ${model_path} --order_model_path ${order_model_path} --gen_type ${gen_type} --gpu_id ${gpuid} --src_lang=${src_lang} --trg_lang=${trg_lang} --data_path=${data_path} --max_seq_length ${max_seq_length}
}

function beam_1iter() {
  python3 -W ignore adaptive_gibbs_sampler_simple_with_predicted_order.py --use_data_length --num_topk_lengths 4 --order_beam=4 --split $split --model_path ${model_path} --order_model_path ${order_model_path} --gen_type ${gen_type} --gpu_id ${gpuid} --src_lang=${src_lang} --trg_lang=${trg_lang} --data_path=${data_path} --max_seq_length ${max_seq_length}
}

function left_right_beam_1iter() {
  python3 -W ignore adaptive_gibbs_sampler_beam_simple.py --alpha 0.0 --beta 0.0 --gamma 1.0 --use_data_length --num_topk_lengths 4 --split $split --model_path ${model_path} --gen_type ${gen_type} --gpu_id ${gpuid}
}

function uniform_greedy_1iter() {
  python3 -W ignore adaptive_gibbs_sampler_simple_with_predicted_order.py --alpha 0.0 --beta 0.0 --gamma 0.0 --uniform --use_data_length --num_topk_lengths 4 --split $split --model_path ${model_path} --gen_type ${gen_type} --gpu_id ${gpuid}
}
